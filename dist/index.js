...
client.on('interactionCreate', async (interaction) => {
    ...
    if (command.commandName === 'gacha') {
        ...
        if (tempGachaCost > currentAmount)
            await command.reply(`Sorry ${interaction.user}, you have ${currentAmount}💰 and a roll costs ${tempGachaCost}💰.`);
        else {
    ...
    if (command.commandName === 'transfer') {
        ...
        if (transferAmount > currentAmount)
            await command.reply(`Sorry ${interaction.user}, you only have ${currentAmount}.`);
        else if (transferAmount <= 0)
            await command.reply(`Please enter an amount greater than zero, ${interaction.user}.`);
        else {
